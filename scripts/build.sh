...
build_all() {
  ...
  echo "===== build and install libevent ====="
  echo "===== build and install libevent =====" >> $SOURCE_DIR/scripts/build.log
  build_and_install "$source_dir" "$target_dir" "deps/libevent" "" 1>> $SOURCE_DIR/scripts/build.log
  echo "===== build and install zookeeper ====="
  echo "===== build and install zookeeper =====" >> $SOURCE_DIR/scripts/build.log
  build_and_install "$source_dir" "$target_dir" "zookeeper/src/c" "" 1>> $SOURCE_DIR/scripts/build.log
  echo "===== build and install server ====="
  echo "===== build and install server =====" >> $SOURCE_DIR/scripts/build.log
  build_and_install "$source_dir" "$target_dir" "server" "--enable-zk-integration --with-libevent=$target_dir --with-zookeeper=$target_dir" 1>> $SOURCE_DIR/scripts/build.log
  echo "===== build and install end ====="
  echo "===== build and install end =====" >> $SOURCE_DIR/scripts/build.log
  ...
  pushd $target_dir/scripts > /dev/null
  ln -s ../lib/python/site-packages/fab fab
  popd > /dev/null
  echo "===== python package install end ====="
}
...
touch $SOURCE_DIR/scripts/build.log
echo "created a build log file in directory $SOURCE_DIR."
